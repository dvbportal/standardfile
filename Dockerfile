...
ENV STANDARDFILE_VERSION=0.1.0
...
RUN apk add --no-cache ca-certificates gnupg openssl libcap jq && \
    ...
    gpg --batch --verify standardfile-${STANDARDFILE_VERSION}-linux-amd64.zip.sig standardfile-${STANDARDFILE_VERSION}-linux-amd64.zip && \
